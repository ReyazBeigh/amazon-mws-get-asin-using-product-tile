...
class MWSClient{
    ...
    private $MarketplaceIds = [
        ...
        'A2VIGQ35RCS4UG'    => 'mws-eu.amazonservices.com',
    ];
    ...
    public function GetCompetitivePricingForASIN($asin_array = [])
    {
     ...
     public function GetLowestOfferListingsForSKU($sku_array = [], $ItemCondition = null)
    {
        ...
        if (isset($response['GetLowestOfferListingsForSKUResult'])) {
            $response = $response['GetLowestOfferListingsForSKUResult'];
            if (array_keys($response) !== range(0, count($response) - 1)) {
                $response = [$response];
            }
        } else {
        ...
        foreach ($response as $product) {
            if (isset($product['Product']['LowestOfferListings']['LowestOfferListing'])) {
                ...
                $array[$sku]= $product['Product']['LowestOfferListings']['LowestOfferListing'];
                //$array[$product['Product']['Identifiers']['MarketplaceSKU']['SKU']] = $product['Product']['LowestOfferListings']['LowestOfferListing'];
            } //else {
                //$array[$product['Product']['Identifiers']['MarketplaceSKU']['SKU']] = false;
            //}
        }
        return $array;
    ...
    public function postProduct($MWSProduct) {
        ...
        $header = ['sku', 'price', 'quantity', 'product-id',
            'product-id-type', 'condition-type', 'condition-note', 'title',
            'ASIN-hint', 'product-tax-code', 'operation-type',
